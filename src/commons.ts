...
export const prometheusQuery = (query: string, legend: string): prometheus.DataqueryBuilder => {
  return new prometheus.DataqueryBuilder().datasource({ uid: '$datasource' }).expr(query).legendFormat(legend)
}

export const tablePrometheusQuery = (query: string, ref: string): prometheus.DataqueryBuilder => {
  return new prometheus.DataqueryBuilder().datasource({ uid: '$datasource' }).expr(query).format(prometheus.PromQueryFormat.Table).refId(ref)
}
...
export const unameStat = (panelTitle: string, panelDescription: string, field: string): StatBuilder => {
  return defaultStat()
    ...
    .withTarget(tablePrometheusQuery('node_uname_info{job=~"integrations/(node_exporter|unix)",cluster=~"$cluster",job=~"$job",instance=~"$instance"}', 'A'))
    .reduceOptions(new common.ReduceDataOptionsBuilder().calcs(['lastNotNull']).fields(`/^${field}$/`))
...
export const defaultTimeseries = (): TimeseriesBuilder => {
  return new TimeseriesBuilder()
    ...
    .legend(new common.VizLegendOptionsBuilder().showLegend(true).placement(common.LegendPlacement.Bottom).displayMode(common.LegendDisplayMode.List))
    .tooltip(new common.VizTooltipOptionsBuilder().mode(common.TooltipDisplayMode.Multi).sort(common.SortOrder.Descending))
    .thresholdsStyle(new common.GraphThresholdsStyleConfigBuilder().mode(common.GraphThresholdsStyleMode.Off))
}
